...
CREATE TABLE tb_albums (
  ...
  FOREIGN KEY (alb_art_id) REFERENCES tb_artists(art_id),
  FOREIGN KEY (alb_gen_id) REFERENCES tb_genres(gen_id)
);
...
CREATE TABLE tb_songs (
  ...
  son_duration FLOAT,
  son_lyrics TEXT,
  FOREIGN KEY (son_alb_id) REFERENCES tb_albums(alb_id)
);

SELECT * FROM tb_songs;